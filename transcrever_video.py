...
st.set_page_config(page_title="Transcrição de Vídeo/Áudio", layout="centered")
st.title("Transcritor de Legendas e Áudio com Whisper")

aba = st.sidebar.radio("Escolha o modo:", ["Legenda (.vtt)", "Áudio/Vídeo (Whisper)"])
...
if aba == "Legenda (.vtt)":
    ...
    idioma = st.selectbox("Idioma da legenda (visual)", list(idiomas_visuais.keys()))
    if vtt_url:
        try:
